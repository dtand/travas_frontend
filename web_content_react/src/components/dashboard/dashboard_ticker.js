...
export default class DashboardTicker extends React.Component {
  ...
  updateInterval(value){
    this.props.updateInterval(this.props.index,value.toLowerCase());
  }
  ...
  getPercentChange(){
    ...
    if(this.getLastClose() === 0 || 
       this.getLastPrice() === 0 ||
       isNaN(this.getLastClose()) ||
       isNaN(this.getLastPrice()) ){
      return 0;
    }
  ...
  getLastPriceText(){
    if(this.props.candles.priceData.length === 0){
      return "NO DATA";
    }
    else if(!this.props.candles.lastPrice.close){
      return this.props.candles.priceData[this.props.candles.priceData.length-1].close;
    }
  ...
  getBorderFromValue(){
    if(this.getLastPrice() > this.getLastClose()) {
      return "style_prevu_kit_sm col-md-3 rounded border-2-success";
    }
    else if(this.getLastPrice() < this.getLastClose()) {
      return "style_prevu_kit_sm col-md-3 rounded border-2-danger";
    }
    return "style_prevu_kit_sm col-md-3 rounded border-2-bottom-primary";
  }
  ...
  render() {
    ...
    let borderClass = "col-md-3 rounded border-2-bottom-primary clickable";
    ...
    return (
        <div className={ borderClass }
          ...
          { this.props.candles.length != 0 &&
            ...
            <h5 className="border-bottom border-secondary text-black"
                style={ HEADER_STYLE }>
            ...
            <div style={ FIRST_ITEM_STYLE }>
              ...
              <span className={ "float-right " + 
                ...
                { this.props.candles.length != 0 ? this.getPercentChange() : "NO DATA" }
              </span>
            </div>
            <div>
              <strong>LAST</strong>
              <span className="float-right"
                    style={ FLOAT_RIGHT_STYLE }>
                { this.props.candles.length != 0 ? this.getLastPriceText() : "NO DATA" }
              </span>
            </div>
            <div style={ LAST_ITEM_STYLE }>
              ...
              <span> 
                { this.props.candles.length != 0 ? parseFloat(this.getLastVolume().toString()) : "NO DATA" }
              </span><br/>
            </div>
